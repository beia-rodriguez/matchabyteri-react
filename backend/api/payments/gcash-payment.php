...
$stmt = $conn->prepare("
    SELECT id, user_id, booking_date, start_time, end_time, booking_type, notes, payment_status
    FROM bookings
    WHERE id = ?
    LIMIT 1
");
...
/* Prevent duplicate payments unless last is rejected */
$stmt = $conn->prepare("SELECT id, status FROM payments WHERE booking_id=? ORDER BY id DESC LIMIT 1");
$stmt->bind_param("i", $bookingId);
$stmt->execute();
...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    /* Build context_json from booking + notes */
    $context = [
        "purpose"      => $purpose,
    ...
    if (is_array($notesDecoded)) {
        foreach (["workshop_id","workshop_title","package"] as $k) {
            if (isset($notesDecoded[$k])) $context[$k] = $notesDecoded[$k];
        }
    }
    ...
    $stmt = $conn->prepare("
        ...
        (user_id, booking_id, purpose, payment_token, reference_no, payer_name, amount, proof_path, status, context_json)
        VALUES
        (?, ?, ?, ?, ?, ?, ?, ?, 'pending', ?)
    ");
    $stmt->bind_param(
    ...
    if ($stmt->execute()) {
        ...
        /* Mark booking payment_status pending */
        $up = $conn->prepare("UPDATE bookings SET payment_status='pending' WHERE id=? AND user_id=?");
        ...
        $stmt->close();
        http_response_code(500);
        echo json_encode(["error" => "Failed to save payment. Please try again."]);
        exit();
    }
} else {
    /* If GET request, just return booking info and GCASH details */
    echo json_encode([
        "success" => true,
